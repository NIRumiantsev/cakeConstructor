  ...
  if (document.querySelector('#root')) {
    ...
    let Constructor = new Vue({
      ...
      methods: {
        ...
        saveAndBuy() {
          localStorage.clear();
          const cakeContainer = document.querySelector('.constructor__cake')
          if (cakeContainer) {
            const cake = cakeContainer.cloneNode(true);
            this.selectedProperties.image = cake.innerHTML;
            const cakeInfo = JSON.stringify(this.selectedProperties);
            localStorage.setItem("cake", cakeInfo);
            console.log(JSON.parse(localStorage.cake))
          }
        }
      },
    })
